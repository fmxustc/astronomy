...
if __name__ == '__main__':
    ...
    for k in catalog.index:
        t0 = time.clock()
        ctl = catalog.ix[k]
        conf1 = '-CATALOG_NAME %s -CHECKIMAGE_TYPE SEGMENTATION -CHECKIMAGE_NAME %s' % (type1_catalog_directory+ctl.NAME1+'_r.txt', type1_seg_directory+ctl.NAME1+'_r.fits')
        conf2 = '-CATALOG_NAME %s -CHECKIMAGE_TYPE SEGMENTATION -CHECKIMAGE_NAME %s' % (type2_catalog_directory+ctl.NAME2+'_r.txt', type2_seg_directory+ctl.NAME2+'_r.fits')
